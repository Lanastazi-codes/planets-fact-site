...
// if (mainHeading.textContent ==="Mercury") {
//     console.log(mainHeading);
// }

// let mainHeadingId = document.getElementById("heading");
...
if (window.innerWidth >= 1025) {
    ...
    // textButtonsWrapper.remove();
    textWrapper.outerHTML = textWrapper.innerHTML; // removing just parent
}

// !
overviewBtn.addEventListener("click", () => {
    let size =
        getComputedStyle(planetPicChild).getPropertyValue("--svgWidthHeigth");
    tl.fromTo(
        ".planet-menu__pic svg",
        { width: 70, height: 70 },
        { width: size, height: size, duration: 1 }
    );
    // tl.from(".planet-menu__pic svg", { width: 70, height: 70, duration: 1.5 });
...
surfaceBtn.addEventListener("click", () => {
    let size =
        getComputedStyle(planetPicChild).getPropertyValue("--svgWidthHeigth");
    if (window.innerWidth >= 670) {
        size = getComputedStyle(planetPicChild).getPropertyValue(
            "--svgWidthHeigthTablet"
        );
    }
    // let percentSize = (size / 10) * 7;
    tl.fromTo(
        ".planet-menu__pic svg",
        { width: 70, height: 70 },
        { width: size, height: size, duration: 1 }
